...
import ImmutableCompare from '../../utils/ImmutableCompare';
...
export default class TreeCheck extends React.Component {
    ...
    handButton = (e) => {
        ...
        let isHide = $(dom).find('div.ant-dropdown').hasClass('hide');
        let isScroll =$(document).find('div.frame-head-wrapper-scroll').length;

        let len = $(document).find('div.ant-dropdown').length;
        for(let i=0; i < len; i++ ){
            $(document).find('div.ant-dropdown').eq(i).addClass('hide')
        }

        if(isHide){
            $(dom).find('div.ant-dropdown').removeClass('hide');
            this.setState({
                isIcon:true
            })
        }else{
            $(dom).find('div.ant-dropdown').addClass('hide');
            this.setState({
                isIcon:false
            })
        }
        ...
        if (isScroll){
            if(isHide){
                $(document).find('div.frame-head-wrapper-scroll').css({
                    overflow: 'visible'
                })
            }else{
                $(document).find('div.frame-head-wrapper-scroll').css({
     ...
     */
    updateTreeList = (data) => {
        let treeData = Immutable.fromJS(data)
        let treeList = List()
        // 递归获取树列表
        const getTreeList = (data, parent = Map()) => {
            data.forEach((node) => {
                node = node.set('parent', parent);
                treeList = treeList.push(node.delete('child'));
    ...
    onCheck = (checkedKeys, e) => {
        let len = 0, lenValue, checkedArrs=[];
        ...
        this.setState({SelectKey: (len == '0')?'全部':(len == '1' && !this.props.getAllNodes)? lenValue :('选中'+ len +'项')})
    }

    onChange = (e) => {
        const value = e.target.value;
        let uniqueExpandedKeys = Map({});
        const {treeList} = this.state
        if (value) {
            // 遍历树列表获取被搜索匹配到的树父id
            treeList.map((item) => {
                if (item.get('label') && item.get('label').indexOf(value) > -1) {
                    uniqueExpandedKeys = uniqueExpandedKeys.merge(item.get('parent'));
                }
            });
        }
        else {
        ...
        this.setState({
            expandedKeys: uniqueExpandedKeys.keySeq().toJS(),
            searchValue: value,
            autoExpandParent: true,
        });
    }
    ...
    renderTreeNodes = (data) => {
        ...
        return data.map((item) => {
            ...
            const title = index > -1 ? (<span>{beforeStr}<span style={{ color: '#f50' }}>{searchValue}</span>{afterStr}</span>) : <span>{item.label}</span>;
            if (item.children) {
                return (
            ...
            if(async)
                return <TreeNode {...{title:title, key:item.value}} isLeaf={!item.hasOwnProperty('child') || item.child.length == 0} />;
            else
        ...
        return <div style={linkStyle} >
            ...
            <span style={{position:'relative', display: 'inline-block',}}>
                <Button title={this.state.SelectKey} style={buttonStyle} onClick={this.handButton}>
                    ...
                    <Icon style={{position:'absolute',right:8,top:10}} type={this.state.isIcon?'up':'down'} />
                </Button>
                <div className="ant-dropdown hide" onClick={this.handeDown} style={{minWidth:200,top:this.props.selectTop,left:0}}>
                    <div className="ant-dropdown-menu">
                        {this.state.isShowSearch && (<Search style={{width:'94%',margin: '2% 3% 0 3%' }}
                                                             placeholder="请搜索......"
...
TreeCheck.defaultProps = {
    ...
    spanName: '',
    onCheckedKeyChange: function () { //选中节点输出
